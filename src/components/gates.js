...
function makeReductionGate({ type, label, color, op, identity, helpText }) {
  registerComponentType({
    type,
    category: 'Bus',
    label,
    color,
    paramsSchema: [
      { key: 'width', label: 'Bitbreite', kind: 'int', min: 1, max: 32, step: 1, default: 16 },
    ],
    pins: (params) => [
      { id: 'in0', label: 'A', dir: 'in', width: params.width ?? 16, side: 'left', order: 0 },
    ...
    evaluate: ({ inputs, params }) => {
      ...
      for (const bit of a) {
        if (bit === CONFLICT) {
          return { outputs: { out: [CONFLICT] }, state: {} };
        }
        if (bit === FLOATING) {
          floating = true;
          continue;
      ...
      if (floating) acc = FLOATING;

      return { outputs: { out: [acc] }, state: {} };
    },
    help: {
      summary: helpText,
      usage: 'Reduziert alle Bits des Eingangs zu einem einzelnen Bit.',
      pins: {
        in0: 'Eingangsbus.',
...
makeReductionGate({
  ...
  op: (a, b) => a | b,
  identity: 0,
  helpText: 'Liefert 1, sobald mindestens ein Bit des Eingangs 1 ist.',
});
...
makeReductionGate({
  ...
  op: (a, b) => a & b,
  identity: 1,
  helpText: 'Liefert 1, wenn alle Bits des Eingangs 1 sind.',
});
...
makeReductionGate({
  ...
  op: (a, b) => a ^ b,
  identity: 0,
  helpText: 'Liefert 1, wenn eine ungerade Anzahl der Eingangsbits 1 ist (Paritätsbit).',
});